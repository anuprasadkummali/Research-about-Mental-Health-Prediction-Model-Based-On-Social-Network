...
# select input
sql = 'SELECT `COL 1`, `COL 46`, `COL 48` FROM `OHQ`;';
cursor.execute(sql);
...
average1 = 3986/4802; # 855
average1 = round(average1, 8);
average2 = 3998/4804; # 865
...
sql = 'ALTER TABLE `features_list` ADD `F69` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F69` = %s WHERE `QN` = %s;'
for i in range(1, size):
