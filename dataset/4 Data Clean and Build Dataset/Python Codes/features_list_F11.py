...
# select input
sql = 'SELECT `COL 1`, `COL 14` FROM `DBQ`;';
cursor.execute(sql);
...
average1 = (546+1350*0.75+2743*0.5+1482*0.25)/6463; #700
...
sql = 'ALTER TABLE `features_list` ADD `F11` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F11` = %s WHERE `QN` = %s;'
for i in range(1, size):
