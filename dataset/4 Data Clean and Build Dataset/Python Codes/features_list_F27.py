...
# select input
sql = 'SELECT `COL 1`, `COL 2`, `COL 4`, `COL 5`, `COL 6` FROM `MCQ`;';
cursor.execute(sql);
...
average1 = 8222/9760; #MCQ010
average1 = round(average1, 8);
average2 = 582/1521; #035
average2 = round(average2, 8);
average3 = 473/937; #040
average3 = round(average3, 8);
average4 = 745/938; #050
...
sql = 'ALTER TABLE `features_list` ADD `F27` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F27` = %s WHERE `QN` = %s;'
for i in range(1, size):
