...
# select input
sql = 'SELECT `COL 1`, `COL 32` FROM `CSQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F59` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F59` = %s WHERE `QN` = %s;'
