...
# select input
sql = 'SELECT `COL 1`, `COL 15` FROM `MCQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F36` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F36` = %s WHERE `QN` = %s;'
