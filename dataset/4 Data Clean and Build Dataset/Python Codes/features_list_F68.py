...
# select input
sql = 'SELECT `COL 1`, `COL 45` FROM `OHQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F68` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F68` = %s WHERE `QN` = %s;'
