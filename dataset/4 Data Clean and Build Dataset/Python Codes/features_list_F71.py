...
# select input
sql = 'SELECT `COL 1`, `COL 2` FROM `DIQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F71` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F71` = %s WHERE `QN` = %s;'
