...
# select input
sql = 'SELECT `COL 1`, `COL 7`, `COL 9` FROM `BPQ`;';
cursor.execute(sql);
...
average1 = 4883/6462; #056
average1 = round(average1, 8);
average2 = 5601/6463; #059
...
sql = 'ALTER TABLE `features_list` ADD `F75` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F75` = %s WHERE `QN` = %s;'
for i in range(1, size):
