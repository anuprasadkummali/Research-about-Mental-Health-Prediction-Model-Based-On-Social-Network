...
# select input
sql = 'SELECT `COL 1`, `COL 5` FROM `HSQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F53` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F53` = %s WHERE `QN` = %s;'
