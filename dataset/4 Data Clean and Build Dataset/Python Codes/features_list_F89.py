...
# select input
sql = 'SELECT `COL 1`, `COL 4`, `COL 5` FROM `IMQ`;';
cursor.execute(sql);
...
average1 = 600/2963; #040
average1 = round(average1, 8);
average2 = 277/2719; #070
...
for i in range(1, size):
    ...
    data2[i][1] = max( float(data2[i][1]), float(data2[i][2]));
#end of loop
...
sql = 'ALTER TABLE `features_list` ADD `F89` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F89` = %s WHERE `QN` = %s;'
for i in range(1, size):
