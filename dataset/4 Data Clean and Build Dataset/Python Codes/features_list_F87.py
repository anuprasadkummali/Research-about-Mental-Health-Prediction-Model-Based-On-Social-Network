...
# select input
sql = 'SELECT `COL 1`, `COL 2` FROM `IMQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F87` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F87` = %s WHERE `QN` = %s;'
