...
# select input
sql = 'SELECT `COL 1`, `COL 10` FROM `KIQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F82` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F82` = %s WHERE `QN` = %s;'
