...
# select input
sql = 'SELECT `COL 1`, `COL 2`, `COL 5`, `COL 8` FROM `PAQ`;';
cursor.execute(sql);
...
average1 = 1172/7147; #PAQ605
average1 = round(average1, 8);
average2 = 2308/7145*0.5; #PAQ620
average2 = round(average2, 8);
average3 = 2047/7146*0.5; #PAQ635
...
for i in range(0, size):
    ...
    data2[i][1] = (decimal.Decimal(data2[i][1]) + decimal.Decimal(data2[i][2]) + decimal.Decimal(data2[i][3]) )/2;
# end of loop
...
sql = 'ALTER TABLE `features_list` ADD `F1` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F1` = %s WHERE `QN` = %s;'
