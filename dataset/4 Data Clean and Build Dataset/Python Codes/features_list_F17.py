...
# select input
sql = 'SELECT `COL 1`, `COL 7` FROM `WHQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F17` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F17` = %s WHERE `QN` = %s;'
