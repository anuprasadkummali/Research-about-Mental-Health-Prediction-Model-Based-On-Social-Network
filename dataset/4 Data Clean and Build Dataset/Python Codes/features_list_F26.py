...
# select input
sql = 'SELECT `COL 1`, `COL 3` FROM `HOQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F26` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F26` = %s WHERE `QN` = %s;'
