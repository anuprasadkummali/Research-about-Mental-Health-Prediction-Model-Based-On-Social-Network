...
# select input
sql = 'SELECT `COL 1`, `COL 3`, `COL 8` FROM `DLQ`;';
cursor.execute(sql);
...
for i in range(1, size):
    ...
    if data2[i][1] == '77777':
        ...
        data2[i][1] = data2[i][1]/34;#34 hrs is considered as full-time in USA
        if data2[i][1] > 1 :
            data2[i][1] = 1;
...
sql = 'ALTER TABLE `features_list` ADD `F3` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F3` = %s WHERE `QN` = %s;'
for i in range(1, size):
