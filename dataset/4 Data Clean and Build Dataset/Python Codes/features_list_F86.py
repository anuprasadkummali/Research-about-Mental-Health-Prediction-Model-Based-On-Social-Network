...
# select input
sql = 'SELECT `COL 1`, `COL 2`, `COL 4` FROM `HEQ`;';
cursor.execute(sql);
...
average1 = 8479/8545; #010
average1 = round(average1, 8);
average2 = 8468/8544; #030
...
sql = 'ALTER TABLE `features_list` ADD `F86` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F86` = %s WHERE `QN` = %s;'
for i in range(1, size):
