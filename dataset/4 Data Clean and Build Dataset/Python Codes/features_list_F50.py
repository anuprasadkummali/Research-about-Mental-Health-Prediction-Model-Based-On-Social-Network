...
# select input
sql = 'SELECT `COL 1`, `COL 10` FROM `HUQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F50` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F50` = %s WHERE `QN` = %s;'
