...
# select input
sql = 'SELECT `COL 1`, `COL 6` FROM `HUQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F47` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F47` = %s WHERE `QN` = %s;'
