...
# select input
sql = 'SELECT `COL 1`, `COL 31` FROM `CSQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F58` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F58` = %s WHERE `QN` = %s;'
