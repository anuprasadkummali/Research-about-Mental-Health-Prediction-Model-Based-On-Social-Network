...
# select input
sql = 'SELECT `COL 1`, `COL 2`, `COL 4` FROM `KIQ`;';
cursor.execute(sql);
...
average1 = 5574/5761; #022
average1 = round(average1, 8);
average2 = 5223/5758; #026
...
sql = 'ALTER TABLE `features_list` ADD `F79` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F79` = %s WHERE `QN` = %s;'
for i in range(1, size):
