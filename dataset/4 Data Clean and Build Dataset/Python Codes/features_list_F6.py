...
for i in range(1, size):
    ...
    if data2[i][2] == '777777':
        data2[i][2] = average2;
    elif data2[i][2] == '999999':
        data2[i][2] = average2;
    elif data2[i][2] == '#NULL!':
        data2[i][2] = average2;
    else:
        data2[i][2] =  float(data2[i][2]);
    ...
    if float(data2[i][1])==0 and float(data2[i][2])==0 and float(data2[i][3])==0 and float(data2[i][4])==0 and float(data2[i][5])==0:
        data2[i][1] = 1-(average1+average3+average4+average5)/(average1+average2+average3+average4+average5);
    else:
        data2[i][1] = 1-(float(data2[i][1])+float(data2[i][3])+float(data2[i][4])+float(data2[i][5])) /(float(data2[i][1])+float(data2[i][2])+float(data2[i][3])+float(data2[i][4])+float(data2[i][5]));
# end of loop
...
sql = 'ALTER TABLE `features_list` ADD `F6` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F6` = %s WHERE `QN` = %s;'
for i in range(1, size):
