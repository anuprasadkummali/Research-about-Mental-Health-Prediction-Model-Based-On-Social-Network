...
# select input
sql = 'SELECT `COL 1`, `COL 11` FROM `DPQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F98` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F98` = %s WHERE `QN` = %s;'
