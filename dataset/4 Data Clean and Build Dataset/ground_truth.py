...
sql = 'ALTER TABLE `dataset` ADD `ground_truth_binary` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `dataset` SET `ground_truth_binary` = %s WHERE `QN` = %s;'
for i in range(1, size):
     cursor.execute(query, (data2[i][2], data2[i][0]));
conn.commit() # very important for update sql server

# create new column in table
sql = 'ALTER TABLE `dataset` ADD `ground_truth` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `dataset` SET `ground_truth` = %s WHERE `QN` = %s;'
for i in range(1, size):
