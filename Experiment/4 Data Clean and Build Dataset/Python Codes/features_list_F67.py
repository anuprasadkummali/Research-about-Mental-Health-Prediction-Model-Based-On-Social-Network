...
# select input
sql = 'SELECT `COL 1`, `COL 39` FROM `OHQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F67` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F67` = %s WHERE `QN` = %s;'
