...
# select input
sql = 'SELECT `COL 1`, `COL 38` FROM `OHQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F66` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F66` = %s WHERE `QN` = %s;'
