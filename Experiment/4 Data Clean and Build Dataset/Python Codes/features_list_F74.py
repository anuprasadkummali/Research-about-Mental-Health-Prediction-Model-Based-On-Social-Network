...
# select input
sql = 'SELECT `COL 1`, `COL 5`, `COL 6` FROM `BPQ`;';
cursor.execute(sql);
...
average1 = 357/2172; #040a
average1 = round(average1, 8);
average2 = 228/1814; #050a
...
sql = 'ALTER TABLE `features_list` ADD `F74` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F74` = %s WHERE `QN` = %s;'
for i in range(1, size):
