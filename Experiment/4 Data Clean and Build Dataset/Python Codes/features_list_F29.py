...
# select input
sql = 'SELECT `COL 1`, `COL 8` FROM `MCQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F29` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F29` = %s WHERE `QN` = %s;'
