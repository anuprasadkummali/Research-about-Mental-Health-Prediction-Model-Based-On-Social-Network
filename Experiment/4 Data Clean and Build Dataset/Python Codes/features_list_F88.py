...
# select input
sql = 'SELECT `COL 1`, `COL 3` FROM `IMQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F88` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F88` = %s WHERE `QN` = %s;'
