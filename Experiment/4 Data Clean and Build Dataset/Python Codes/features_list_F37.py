...
# select input
sql = 'SELECT `COL 1`, `COL 19` FROM `MCQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F37` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F37` = %s WHERE `QN` = %s;'
