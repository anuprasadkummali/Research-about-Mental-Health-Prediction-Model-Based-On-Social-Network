...
# select input
sql = 'SELECT `COL 1`, `COL 6` FROM `DEQ`;';
cursor.execute(sql);
...
sql = 'ALTER TABLE `features_list` ADD `F84` VARCHAR(10) NOT NULL;';
cursor.execute(sql);

# store into correct table in DB
query = 'UPDATE `features_list` SET `F84` = %s WHERE `QN` = %s;'
